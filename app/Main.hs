...
agdaCoreCompile env _ _ def = do
  ...
  case convert ntcg def of
    ...
    Right def'-> do   
      reportSDoc "agda-core.check" 1 $ text $ "Got core definition: " <> show def'
  ...
  for_ (zip (iterate Scope.inThere Scope.inHere) defs) $ \(i, def) -> do
    case def of
      ...
      Right Core.Definition{ defName, theDef = Core.FunctionDefn funBody, defType } -> do
        ...
        reportSDoc "agda-core.check" 1 $ text $ "Checking termination of " <> defName <> "Inferred index: " <> show i <> ",\n body :" ++ show funBody
      ...
      Right Core.Definition{ defName } ->
        reportSDocWarning "agda-core.check" 1 $ text $ "Skiped " <> defName <> " : not a function"
